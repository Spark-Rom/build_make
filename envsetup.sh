...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^spark_") ; then
        ...
        export BUILD_NUMBER=$( (date +%s%N ; echo $SPARK_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        SPARK_BUILD=
    fi
    export SPARK_BUILD
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        pushd $T > /dev/null
        $T/vendor/spark/build/tools/roomservice.py $product
        popd > /dev/null
        check_product $product
    else
        T=$(gettop)
        pushd $T > /dev/null
        $T/vendor/spark/build/tools/roomservice.py $product true
        popd > /dev/null
    fi
    if [ $? -ne 0 ]
    then
